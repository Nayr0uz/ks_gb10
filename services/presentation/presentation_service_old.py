...
# ==========================#
# DEPRECATED: Old helper functions (replaced by unified prompt)
# ==========================#
# The following functions are no longer used since _extract_topics_with_multiple_methods
# now handles all extraction in a single LLM call for better performance.
# Kept here for reference but not called.
#
# - _extract_topics_from_chunks() 
# - _extract_topics_from_title()
# - _extract_keywords_from_content()
# - _extract_sections_programmatically()
...
#     chunks: List[str]
# ) -> List[str]:
#     """Extract main topics by analyzing document chunks"""
#     if not chunks:
#         return []
#     
#     # Sample top chunks for analysis
#     sample_chunks = chunks[:10]
#     combined_text = "\n\n".join(sample_chunks)
#     
#     prompt = f"""You are a financial document analyst. 
#     Analyze this banking document excerpt and identify 6-8 MAIN TOPICS or PRODUCT CATEGORIES mentioned.
# 
# Document:
# ---
# {combined_text[:3000]}
# ---
# 
# Return ONLY a comma-separated list of main topics (one topic per line). Be specific and concise.
# 
# Topics:"""
#     
#     try:
...
#             None, lambda: client.generate(model=model_name, prompt=prompt)
#         )
#         topics = [t.strip() for t in resp["response"].strip().split('\n') if t.strip()]
#         return [t for t in topics if len(t) > 2][:8]
#     except Exception as e:
#         logger.error(f"Error in chunk extraction: {e}")
#         return []


# async def _extract_topics_from_title(
#     client: Client,
#     model_name: str,
#     title: str,
#     chunks: List[str]
# ) -> List[str]:
#     """Extract topics by analyzing document title and related sections"""
#     if not title or not chunks:
#         return []
#     
#     # Find chunks that relate to the title
#     related_chunks = [c for c in chunks[:5] if any(word in c.lower() for word in title.lower().split())]
#     if not related_chunks:
#         related_chunks = chunks[:3]
#     
#     content = "\n\n".join(related_chunks)
#     
#     prompt = f"""Based on the document title "{title}" and its content, what are the 5-7 KEY TOPICS or SECTIONS covered?
# 
# Title: {title}
# 
# Content:
...
# ---
# 
# Return ONLY a comma-separated list of topics. Focus on what the document teaches or explains about the title.
# 
# Topics:"""
#     
#     try:
...
#             None, lambda: client.generate(model=model_name, prompt=prompt)
#         )
#         topics = [t.strip() for t in resp["response"].strip().split(',') if t.strip()]
#         return [t for t in topics if len(t) > 2][:7]
#     except Exception as e:
#         logger.error(f"Error in title extraction: {e}")
#         return []


# async def _extract_keywords_from_content(
#     client: Client,
#     model_name: str,
#     chunks: List[str]
# ) -> List[str]:
#     """Extract important keywords/topics that represent the document's focus"""
#     if not chunks:
#         return []
#     
#     sample = "\n\n".join(chunks[:8])
#     
#     prompt = f"""Extract 8-10 IMPORTANT KEYWORDS or CONCEPTS from this banking document that would be suitable for presentation topics.
# Focus on financial terms, product names, services, features, or key concepts.
# 
# Document:
# ---
# {sample[:3000]}
# ---
# 
# Return ONLY a comma-separated list of keywords. Make them specific and meaningful.
# 
# Keywords:"""
#     
#     try:
...
#             None, lambda: client.generate(model=model_name, prompt=prompt)
#         )
#         keywords = [k.strip() for k in resp["response"].strip().split(',') if k.strip()]
...
# def _extract_sections_programmatically(chunks: List[str]) -> List[str]:
#     """Extract potential sections/topics by analyzing text patterns"""
#     topics = []
#     
#     # Look for common section patterns
#     section_patterns = [
#         r'^#{1,3}\s+(.+)$',  # Markdown headers
#         r'^([A-Z][a-z\s]+):',  # Capitalized headers with colon
...
#                     if len(topic) > 2 and topic.lower() not in [t.lower() for t in topics]:
#                         topics.append(topic)
#                         break
#     
#     return topics[:10]
...
#     topics: List[str]
# ) -> List[str]:
#     """Use LLM to semantically cluster and refine topics"""
#     if not topics:
#         return []
#     
#     # Deduplicate first
#     unique_topics = list(set([t.strip() for t in topics if t.strip()]))
#     
#     if len(unique_topics) <= 10:
#         return unique_topics
#     
#     prompt = f"""You have extracted these topics from a banking document:
# 
# {chr(10).join(f"- {t}" for t in unique_topics[:30])}
# 
# Please consolidate these into 10-12 MAIN topics by:
# 1. Grouping similar/related topics
# 2. Removing duplicates
# 3. Keeping the most important ones
# 
# Return ONLY the final list as comma-separated topics.
# 
# Final Topics:"""
#     
#     try:
...
#         )
#         topics = [t.strip() for t in resp["response"].strip().split(',') if t.strip()]
#         return [t for t in topics if len(t) > 2][:12]
#     except Exception as e:
#         logger.error(f"Error in semantic clustering: {e}")
#         return unique_topics[:12]


# async def _deduplicate_and_rank_topics(topics: List[str]) -> List[str]:
...
async def _find_best_document_by_embedding(
    ...
    try:
        ...
        for i, tok in enumerate(tokens, start=1):
            params.append(f"%{tok}%")
            conds.append(f"(title ILIKE ${i} OR file_name ILIKE ${i})")
        
        # Use OR instead of AND for flexible matching
        query = (
            "SELECT id, title, file_name FROM documents WHERE "
            + " OR ".join(conds)
            + " ORDER BY created_at DESC LIMIT 1"
        )
        
        doc = await db_conn.fetchrow(query, *params)
        if doc:
        ...
        doc = await db_conn.fetchrow(
            """SELECT id, title, file_name FROM documents 
               WHERE title ILIKE $1 ESCAPE '\\' OR file_name ILIKE $1 ESCAPE '\\\\'
               ORDER BY created_at DESC LIMIT 1""",
            like_query
        )
        if doc:
        ...
        doc = await db_conn.fetchrow(
            "SELECT id, title, file_name FROM documents ORDER BY created_at DESC LIMIT 1"
        )
...
"""
    ...
    try:
        loop = asyncio.get_event_loop()
        resp = await loop.run_in_executor(
            None, lambda: client.generate(model=model_name, prompt=prompt, format="json")
        )
        outline = json.loads(resp["response"])
...
You MUST use ONLY the key points provided below.
Do NOT add, invent, generate, or enhance any facts.
Do NOT add details not in the key points provided.
Your job is ONLY to format the provided facts into a slide.
...
Each slide focuses on a single topic with maximum 6 bullet points.
Use ONLY the facts provided below - nothing more, nothing less.

# SLIDE CONTENT TO FORMAT
Product/Service Name: "{slide_title}"

Key Points (USE ONLY THESE - DO NOT ADD MORE):
{points_str}

# YOUR TASK - FORMATTING ONLY (NO CONTENT GENERATION)
...
6) Do NOT reword, expand, or enhance the points
7) Do NOT add new information or facts
8) Do NOT add examples or details not in the provided key points
9) Just clean up formatting if bullet marker is missing
10) NO narrative text, NO additions - ONLY format the provided facts
...
Output the formatted slide now. Use ONLY the points provided above. Do not add any new facts or information.
"""
...
def _calculate_optimal_slides(
    ...
    IMPORTANT: Each topic gets its own slide (one topic per slide maximum).
    """
    ...
    if num_topics == 0:
        ...
        # Each topic gets its own slide - create individual groups
        # topic_groups will be: [[0], [1], [2], ...] where each list contains one topic index
        topic_groups = [[i] for i in range(num_topics)]
        
        # Respect requested slide count: adjust if we have more topics than requested content slides
        num_content_slides = user_requested_slides - 2  # Exclude title and conclusion
        
        if len(topic_groups) > num_content_slides:
            # If we have more topics than requested slides, we need to split some slides
            # Or reduce the number of topics shown
            # For now, just use all topics (will create more slides than requested)
            print(f"⚠️  Warning: {num_topics} topics exceed requested {num_content_slides} content slides")
        
        total_slides = len(topic_groups) + 2  # Add title + conclusion
...
async def generate_presentation_content_streaming(
    ...
    *,
    ...
    # Initialize LLM
    ollama_client = Client(host=os.getenv("OLLAMA_BASE_URL", "http://ollama:11434"))
    ...
    async with db_manager.get_connection() as conn:
        best_doc = await _find_best_document_by_embedding(row['title'], conn)
            ...
            async with db_manager.get_connection() as conn:
                await conn.execute(
                    """UPDATE presentations 
                       SET status = $1, content = $2, updated_at = NOW()
                       WHERE id = $3""",
                    "failed",
    ...
    try:
        async with db_manager.get_connection() as conn:
            await conn.execute(
                """UPDATE presentations 
                   SET status = $1, output_file_path = $2, content = $3 
                   WHERE id = $4""",
                "completed",
                public_url,
                content_str,
